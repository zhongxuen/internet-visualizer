...
explorer grid on the home page, the shared module header, the skip link, the settings
menu, the error body, and the safety badge.
...
- **Registry-driven, never hardcoded.** `TopNav`, `ModuleGrid`, and `ModuleChrome` read
  ...
  list of module ids. Chapter membership lives in `MODULE_CHAPTERS`, not in the nav.
- **Modules supply content, never chrome.** Breadcrumb, title, question, level, the
  words-you'll-meet chips, safety badge, and the explanation-panel slot all belong to `ModuleChrome`. A module route
